...
/**
 *	@brief	USART4 SendData
 */
void UART4_SendData(uint8_t *Data,uint16_t Size)
{
...
void UART_SendData(drv_uart_t *drv,uint8_t *txData,uint16_t size)
{
	...
	else if(drv->id == DRV_UART4)
		UART4_SendData( txData, size);
	else if(drv->id == DRV_UART5)
		UART5_SendData( txData, size);
}

/* rxData Handler [Weak] functions -------------------------------------------*/
/**
 *	@brief	[__WEAK] 需要在Protocol Layer中实现具体的 USART1 处理协议
 */
__WEAK void USART1_rxDataHandler(uint8_t *rxBuf)
{	
}
...
/**
 *	@brief	[__WEAK] 需要在Protocol Layer中实现具体的 USART2 处理协议
 */
__WEAK void USART2_rxDataHandler(uint8_t *rxBuf)
{	
}
...
/**
 *	@brief	[__WEAK] 需要在Protocol Layer中实现具体的 USART4 处理协议
 */
__WEAK void USART4_rxDataHandler(uint8_t *rxBuf)
{	
}
...
/**
 *	@brief	[__WEAK] 需要在Protocol Layer中实现具体的 USART5 处理协议
 */
__WEAK void USART5_rxDataHandler(uint8_t *rxBuf)
{	
}
