...
static void brake_motor_update(motor_t *motor, uint8_t *rxBuf);
...
motor_t	motor[] = {
	...
	[BRAKE] = {
		.info = &motor_info[BRAKE],
		.driver = &motor_driver[BRAKE],
		.init = motor_init,
		.update = brake_motor_update,
		.check = motor_check,
		.heart_beat = motor_heart_beat,
		.work_state = DEV_OFFLINE,
		.id = DEV_ID_BRAKE,
...
static void brake_motor_update(motor_t *dial_motor, uint8_t *rxBuf)
{
	motor_info_t *motor_info = dial_motor->info;
