...
//线性跟随系数Kp
float vis_pitch_corret_kp = 0.7;
float vis_yaw_corret_kp = 0.7;
...
//	vision.pitch_target_predict_speed = 0;
//	vision.yaw_target_predict_speed = 0;	
...
//		//卡尔曼
//		vision.pitch_angle_raw = KalmanFilter(&vision_pitch_raw_filtered ,vision.rx_data->RxData->pitch_angle / 360.f * 8191);
//		vision.yaw_angle_raw = KalmanFilter(&vision_yaw_raw_filtered, vision.rx_data->RxData->yaw_angle / 360.f * 8191);
				
		
		vision.distance_filtered = KalmanFilter(&vision_distance_get, vision.rx_data->RxData->distance);  //更新距离
...
//	vision_sensor_info.TxPacket.TxData.pitch_measure = imu_sensor.info->pitch;
	...
	vision_sensor_info.TxPacket.TxData.attack_color = leader_info.TxPacket.ctrl_mode.attack_colour + 1;   //识别颜色
	vision_sensor_info.TxPacket.TxData.time_stamp = HAL_GetTick();               //时间戳
	vision_sensor_info.TxPacket.TxData.anti_gyro = 0;         //反陀螺标志，视觉处理固定发送
	vision_sensor_info.TxPacket.TxData.bullet_speed = 30;	       //射速
	
	vision_sensor_info.TxPacket.FrameHeader.sof = VISION_SOF;	    //帧头
	vision_sensor_info.TxPacket.FrameHeader.cmd_id = CMD_AIM_AUTO;   //开启自瞄
