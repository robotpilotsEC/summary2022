...
#include "rp_math.h"
#include "drv_haltick.h" //DEBUG:测试发送间隔
 ...
 *	@brief	CAN2 初始化
 */
uint32_t js_can2_esr;
uint32_t js_can2_msr;
uint32_t js_can2_tsr;
void CAN2_Init(void)
{
	CAN_FilterTypeDef sFilterConfig;
 ...
 *	@brief	通过CAN发送数据
 */
uint32_t can_now, can_last, can_gap;
...
//	{
//        js_can2_esr = hcan2.Instance->ESR;
//        js_can2_msr = hcan2.Instance->MSR;    
//        js_can2_tsr = hcan2.Instance->TSR;
